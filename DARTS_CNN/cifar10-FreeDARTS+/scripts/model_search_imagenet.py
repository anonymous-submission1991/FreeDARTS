...
class Network(nn.Module):
  ...
  def _initialize_alphas(self):
    ...
    self.alphas_normal = Variable(5e-4*torch.randn(k, num_ops).cuda(), requires_grad=True)
    self.alphas_reduce = Variable(5e-4*torch.randn(k, num_ops).cuda(), requires_grad=True)

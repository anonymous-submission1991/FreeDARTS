...
import torch.optim
from utils_proxylessNAS import *
...
import sys
sys.path.append("../..")
from utils import get_train_dataloader, DataIterator, get_val_dataloader

class RunConfig:

    def __init__(self, train_dir, test_dir, n_epochs, init_lr, gpu, lr_schedule_type, lr_schedule_param,
        ...
        # Prepare data
        train_loader = get_train_dataloader(self.train_dir, self.train_batch_size, shuffle=True)
...
class RunManager:
    ...
    def __init__(self, path, net, run_config: RunConfig, out_log=True, measure_latency=None):
        ...
        print('device={}, batch_size={}, train_iters={}'.format(\
            self.device, run_config.train_batch_size, run_config.train_iters))
        time.sleep(10)
        # net info
    ...
    def net_flops(self):
        data_shape = [1] + list(self.run_config.data_shape)
    ...
    def net_latency(self, l_type='gpu4', fast=True, given_net=None):
        ...
        data_shape = [batch_size] + list(self.run_config.data_shape)
    ...
    def validate(self, is_test=True, net=None, use_train_mode=False, return_top5=False):
        ...
        # noinspection PyUnresolvedReferences
        with torch.no_grad():
            for i in range(self.run_config.val_iters):
                images, labels = data_loader.next()
                images = Variable(images, requires_grad=False).to(self.device)
                ...
                if (i % self.run_config.print_frequency == 0 or i + 1 == self.run_config.val_iters):
                    if is_test:
                        prefix = 'Test'
                    else:
                        prefix = 'Valid'
                    test_log = prefix + ': [{0}/{1}]\t'\
                        ...
                        format(i, self.run_config.val_iters - 1, batch_time=batch_time, loss=losses, top1=top1)
                    if return_top5:
    ...
    def train_one_epoch(self, adjust_lr_func, train_log_func):
        ...
        train_dataprovider = self.run_config.train_loader
        for _ in range(self.run_config.train_iters):
            data_time.update(time.time() - end)
            new_lr = adjust_lr_func(i)
            iamges, labels = train_dataprovider.next()
            images = Variable(images, requires_grad=False).to(self.device)
            labels = Variable(labels, requires_grad=False).to(self.device)
